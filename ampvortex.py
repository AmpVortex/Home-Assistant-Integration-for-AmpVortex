"""Module for working with AmpVortex device"""
...
class AmpVortexClient:
    """Class for working with AmpVortex device"""

    async def can_connect_to_ampvortex(self, ip_address: str):
        """Verify connectivity to a compatible AmpVortex device"""
        logger.debug(f"Verifying connectivity to AmpVortex with ip_address={ip_address}")
        return True
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(f"http://{ip_address}/api/{api_version}/zones/{zone_id}/volume", json = { "volume": volume}) as response:
                    if response.status != 200:
                        ...
                        logger.debug("AmpVortex set_zone_volume get response 200")
                        return volume
        except aiohttp.ClientError as exc:
            raise UnexpectedException from exc
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(f"http://{ip_address}/api/{api_version}/zones/{zone_id}/input", json = input_str) as response:
                    if response.status != 200:
                        ...
                        logger.debug("AmpVortex set_zone_input get response 200")
                        return str
        except aiohttp.ClientError as exc:
            raise UnexpectedException from exc
            ...
            async with aiohttp.ClientSession() as session:
                async with session.put(f"http://{ip_address}/api/{api_version}/inputs/{input_id}/type", json =  { "type": type }) as response:
                    if response.status != 200:
                        ...
                        logger.debug("AmpVortex set_input_type get response 200")
                        return str
        except aiohttp.ClientError as exc:
            raise UnexpectedException from exc
