"""Config flow for AmpVortex integration."""
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]):
    ...
    hub = AmpVortexHub(hass, data[CONF_HOST])

    if not await hub.verify_connection():
        raise CannotConnect

    LOGGER.debug("Successfully reached the AmpVortex amplifier on the network")
    return await hub.get_devices()


class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for AmpVortex."""
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            try:
                ...
                LOGGER.exception("Failed to reach AmpVortex amplifier on the network")
                ...
                LOGGER.debug("AmpVortex devices: %s. Registering with %s", info, info[0])
    ...
    async def async_step_reauth(self, user_input: dict[str, Any]) -> FlowResult:
        ...
        hub: AmpVortexHub = self.hass.data[DOMAIN][self.reauth_entry.entry_id]["hub"]
        
        await hub.get_connection_info()
        LOGGER.debug("Successfully connected to the AmpVortex amplifier")
