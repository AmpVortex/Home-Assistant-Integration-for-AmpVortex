...
from .hub import AmpVortexHub, AmpVortexDevice
...
async def async_setup_entry(
    ...
    hub: AmpVortexHub = hass.data[DOMAIN][config_entry.entry_id]["hub"]
    coordinator = hass.data[DOMAIN][config_entry.entry_id]["coordinator"]

    entities = []    
    for amp_id in hub.ampvortexs:
        amp = hub.ampvortexs[amp_id]
...
class AmpVortexMediaPlayerBase(CoordinatorEntity, MediaPlayerEntity):
    """Base class for our zone media players"""

    _attr_has_entity_name = True

    def __init__(self, amp: AmpVortexDevice, coordinator, config_entry) -> None:
...
class Zone(AmpVortexMediaPlayerBase):
    """Zone media player"""

    device_class = MediaPlayerDeviceClass.SPEAKER

    def __init__(self, amp: AmpVortexDevice, coordinator, config_entry, zone_id) -> None:
        """Initialize the sensor."""
...
class InputMediaPlayer(AmpVortexMediaPlayerBase):
    """Input media player"""
    
    device_class = MediaPlayerDeviceClass.RECEIVER
    
    def __init__(self, amp: AmpVortexDevice, coordinator, config_entry, input_id, ip_address) -> None:
